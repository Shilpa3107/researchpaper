...
include('db_connection.php'); // Ensure db_connection.php connects to your database

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['remarks']) && isset($_POST['srNo']) && isset($_POST['submit_remarks_research'])) {
    $srNo = $_POST['srNo'];
    ...
    // Fetch Employee ID, paper title, and Faculty from researchpapersbyfaculty table
    ...
    if ($stmt->fetch()) {
        ...
        echo "<script>alert('Record not found in researchpapersbyfaculty'); location.replace('index1admin.php');</script>";
    }
} else {
    echo "<script>alert('Invalid request'); location.replace('index1admin.php');</script>";
}
